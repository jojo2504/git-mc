...
3. Put your server files (server files + modpack) in `git-mc/server/`, (i.e. `git-mc/server/myserver`)
4. Put the name of your server in `settings.json` 
5. Run the server once manually to generate the EULA and world files, and accept the EULA (`eula=true` in `eula.txt`).
...
3. Run the server once manually to generate the EULA, and accept the EULA (`eula=true` in `eula.txt`).
3. Run the server
